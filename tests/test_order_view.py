...
def test_run_order_approval_menu_lists_approves_and_rejects_orders(tmp_path, monkeypatch, capsys):
    order_repository = OrderRepository(str(tmp_path / "orders.json"))
    sample_repository = SampleRepository(str(tmp_path / "samples.json"))
    order_controller = OrderController(order_repository, sample_repository)
    ...
    order_a = order_controller.place_order("S-001", "홍길동", 5, date(2026, 4, 16))
    order_b = order_controller.place_order("S-001", "김철수", 10, date(2026, 4, 16))

    inputs = iter(["1", "2", order_a.id, "3", order_b.id, "4"])
    monkeypatch.setattr("builtins.input", lambda *args: next(inputs))
