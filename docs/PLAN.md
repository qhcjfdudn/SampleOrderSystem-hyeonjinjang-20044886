...
- [ ] 시료 등록 (시료 ID/이름/평균 생산시간/수율 입력) (PRD 5.2)
- [ ] 시료 조회 (전체 목록 + 현재 재고 표시)
- [ ] 시료 검색 (이름 등 속성 검색)

## 4단계 — 시료 주문

- [ ] 시료 예약 입력(시료 ID/고객명/주문 수량) 처리, 상태 `RESERVED`로 생성 (PRD 5.3)
...
- [ ] 부족분(주문 수량 − 재고) / 실생산량(ceil(부족분/수율)) / 총 생산 시간(평균 생산시간 × 실생산량) 계산 (PRD 5.5)
- [ ] 생산 큐 FIFO 처리 및 대기 목록 조회
- [ ] 생산 완료 시 주문 상태 `PRODUCING` → `CONFIRMED` 전환, 재고 반영
- [ ] 생산 현황(현재 생산 중인 시료 정보) 표시

## 7단계 — 출고 처리

- [ ] `CONFIRMED` 주문 목록 표시 및 출고 실행, 상태 `RELEASE` 전환 (PRD 5.6)
...
- [ ] `main.py`에서 메인 메뉴(시료 관리/시료 주문/주문 승인·거절/모니터링/생산 라인 조회/출고 처리) 루프 연결 (PRD 5.1)
- [ ] 메인 메뉴 요약 정보(등록 시료 수, 총 재고, 전체 주문 건수, 생산라인 대기 건수) 표시
