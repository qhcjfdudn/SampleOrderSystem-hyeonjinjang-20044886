...
주요 기능 목록, 주문 상태 흐름, 설치/실행/테스트 명령어는 [README.md](README.md)를 참고하세요
(중복 방지를 위해 이 파일에는 코드 작업에 필요한 설계 규칙만 남깁니다).
...
- 재고 차감은 주문이 `CONFIRMED`로 전환되는 시점에만 이뤄진다 (즉시 승인이든 생산 완료 후 전환이든
  ...
  - 생산 완료 시(`ProductionLineController.complete_current_production`) 재고는
    `기존 재고 + 실생산량 − 주문 수량`으로 갱신된다. 실생산량은 온전히 재고에 더해지지만, 그 주문이
    즉시 소비하는 수량이 같은 시점에 함께 빠지므로 완료 후 재고는 "이번 생산으로 남은 여유분"만
    보인다 — 재고가 실생산량만큼 그대로 누적되는 것으로 착각하기 쉬우니 주의 (예: 재고 5, 주문 20,
    실생산량 17 → 재고는 5+17=22가 아니라 5+17-20=2가 된다. 오버셀링 방지를 위한 의도된 동작).
...
- **메인 메뉴 항목 순서**: 예시 UI는 `[4] 모니터링 [5] 생산 라인 조회 [6] 출고 처리` 순서이나, 실제
  구현은 `4=생산 라인 조회, 5=출고 처리, 6=모니터링` 순서다 (README.md 기능 표 순서를 따름, PRD 5.1
  표 순서와도 다름).
