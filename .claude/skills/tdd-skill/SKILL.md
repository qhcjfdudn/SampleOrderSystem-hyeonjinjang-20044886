...
digraph tdd_cycle {
    rankdir=LR;
    plan [label="PLAN.md\n작성 (기존 내용 삭제)", shape=note, style=filled, fillcolor="#fff2cc"];
    red [label="RED\n실패하는 테스트 작성", shape=box, style=filled, fillcolor="#ffcccc"];
    verify_red [label="제대로\n실패하는지 확인", shape=diamond];
    review_red [label="사용자 검토\n(RED)", shape=diamond, style=filled, fillcolor="#fff2cc"];
    commit_red [label="커밋\n(테스트 + PLAN.md)", shape=box, style=filled, fillcolor="#e6e6e6"];
    green [label="GREEN\n최소한의 코드", shape=box, style=filled, fillcolor="#ccffcc"];
    verify_green [label="통과 확인\n전체 그린", shape=diamond];
    review [label="REVIEW\n정리", shape=box, style=filled, fillcolor="#ccccff"];
    review_gate [label="사용자 검토\n(REVIEW)", shape=diamond, style=filled, fillcolor="#fff2cc"];
    commit_review [label="커밋\n(구현)", shape=box, style=filled, fillcolor="#e6e6e6"];
    next [label="다음", shape=ellipse];

    plan -> red;
    red -> verify_red;
    verify_red -> review_red [label="예"];
    verify_red -> red [label="잘못된\n실패"];
    review_red -> commit_red [label="승인"];
    review_red -> red [label="반려"];
    commit_red -> green;
    green -> verify_green;
    verify_green -> review [label="예"];
    verify_green -> green [label="아니오"];
    review -> verify_green [label="그린 유지"];
    review -> review_gate;
    review_gate -> commit_review [label="승인"];
    review_gate -> review [label="반려"];
    commit_review -> next;
    next -> plan;
}
```

### RED - PLAN.md 작성 및 실패하는 테스트 작성

이번 RED 단계에서 진행할 내용을 `PLAN.md`에 작성하라. 기존 `PLAN.md`가 있다면 그 내용을 삭제하고 새로 작성하라 (이전 단계의 계획을 이어 붙이지 말 것).

`PLAN.md`에는 최소한 다음을 포함하라:
- 이번에 구현할 동작(behavior) 한 가지
- 작성할 테스트의 이름과 검증 내용
- 예상되는 최소 구현 방향 (GREEN 단계에서 참고용)

PLAN.md 작성 후, 무엇이 일어나야 하는지를 보여주는 최소한의 테스트 한 개를 작성하라.
...
테스트가 제대로 실패하는 것을 확인했다면, 사람 파트너에게 다음을 검토받아라:
- `PLAN.md`의 내용이 이번 단계의 목표를 정확히 반영하는가
- 작성한 실패하는 테스트가 계획과 일치하는가

**승인받지 못했다면:** GREEN 단계로 넘어가지 말라. 피드백을 반영해 `PLAN.md`와 테스트를 다시 작성하고, 다시 검토를 요청하라.

**승인받았다면:** `PLAN.md`와 실패하는 테스트를 커밋하라. 그 다음에만 GREEN 단계로 진행하라.
...
정리가 끝났다면 사람 파트너에게 다음을 검토받아라:
- GREEN 단계에서 작성한 구현이 `PLAN.md`의 계획과 일치하는가
- 정리(REVIEW) 과정에서 동작이 바뀌지 않았는가 (테스트는 계속 그린인가)

**승인받지 못했다면:** 커밋하지 말라. 피드백을 반영해 다시 정리하고, 다시 검토를 요청하라.

**승인받았다면:** 구현 코드를 커밋하라. 그 다음에만 다음 RED 단계로 진행하라.

### 반복

다음 기능에 대한 `PLAN.md`를 새로 작성하고, 다음 실패하는 테스트를 작성하라.
...
- 동작: 빈 이메일 제출 시 에러를 반환한다
- 테스트: test_rejects_empty_email
- 예상 구현: submit_form에서 email이 공백뿐이면 {"error": "Email required"} 반환
```
...
**RED 리뷰**
사람 파트너에게 `PLAN.md`와 실패하는 테스트를 검토받는다. 승인 후 `PLAN.md` + 테스트를 커밋한다.
...
**REVIEW**
여러 필드에 대한 검증이 필요해지면 검증 로직을 추출한다. 정리 후 사람 파트너에게 구현을 검토받고, 승인 후 커밋한다.

## 검증 체크리스트

작업을 완료(complete)로 표시하기 전에:

- [ ] 이번 RED 단계의 `PLAN.md`를 작성했다 (기존 내용은 삭제하고 새로 작성)
- [ ] 모든 새 함수/메서드에 테스트가 있다
- [ ] 각 테스트가 실패하는 것을 직접 보고 구현했다
- [ ] 각 테스트가 예상한 이유로 실패했다 (오타가 아니라 기능 부재로)
- [ ] RED 단계에서 사람 파트너의 검토를 받고 승인받았다
- [ ] 승인된 `PLAN.md`와 실패하는 테스트를 커밋했다
- [ ] 각 테스트를 통과시키는 최소한의 코드를 작성했다
- [ ] 모든 테스트가 통과한다
- [ ] 출력이 깨끗하다 (에러, 경고 없음)
- [ ] 테스트가 실제 코드를 사용한다 (mock은 불가피할 때만)
- [ ] 엣지 케이스와 에러 케이스가 커버되어 있다
- [ ] REVIEW 단계에서 사람 파트너의 검토를 받고 승인받았다
- [ ] 승인된 구현을 커밋했다
